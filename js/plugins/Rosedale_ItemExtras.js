...
* @author Chaucer
*
* @plugindesc | Scene Item Extras : Version - 1.0.0 | Scene for extra item variables.
*
...
//=============================================================================
  ...
  Imported['Scene Item Extras'.toUpperCase()] = true;
//=============================================================================
  var Chaucer = Chaucer || {};
  Chaucer.itemExtras = {};
//=============================================================================
...
//=============================================================================
class Scene_ItemExtras extends Scene_Base
{ // Scene_ItemExtras
...
//=============================================================================
  itemContainerRect()
  { // return a rectangle for the item container .
//=============================================================================
...
//=============================================================================
  createItemsPanel()
  { // create the panel that lists all items.
//=============================================================================

    const rect = this.itemContainerRect();

    this._itemContainer = new ContainerField( rect );
...
//=============================================================================
    ...
    const dir = path.join( base, 'data', 'Items.json' )
    let string = fs.readFileSync( dir, { encoding:'utf8', flag:'r' } );
    const data = JsonEx.parse( string );

    data.shift();

    const extras = this.getItemExtras();

    for ( let i = 0, l = data.length; i < l; i++ ) {
      const item = data[i];
      item.extras  = extras[i] || {
        motion: 'item',
        type: 'melee',
        useOnPickup: false,
        projectile: null,
...
//=============================================================================
    ...
    const fileDir = path.join( base, 'data', 'abs', 'Items.json' )
...
//=============================================================================
  createItemList()
  { // create the list of all items in the database.
//=============================================================================

    this._items = this.getProjectItems();

    const container = this._itemContainer;
    const width = container.width - 2;

    this._itemLabel = new LabelField( 'ITEMS' );

    this._itemLabel.x = Math.round( width - this._itemLabel.width ) / 2;
    this._itemLabel.y = 8;

    this._itemList = new ListField( width, 24, this._items  )

    this._itemList.x += 1;
    this._itemList.y += container.height - this._itemList.height - 1;

    container.addElement( this._itemLabel );
    container.addElement( this._itemList );

    this._itemList.setOkHandler( this.onItemSelect.bind( this ) );
...
//=============================================================================
    ...
    const extras = this.allItems().map( n => n.extras );
    extras.unshift( null );
...
//=============================================================================
    ...
    let directory = path.join( folder, 'Items.json' );


    const data = ( $gameTemp._items ).map( n => n.extras );
    data.unshift( null );
...
//=============================================================================
    ...
    const y = 0;
    const width = Graphics.width / 4;
    const height = Graphics.height / 3.075;
...
//=============================================================================
  item()
  { // return the currently selected item/items.
//=============================================================================

    return this._itemList.item();
...
//=============================================================================
  createUseOnPickupField()
  { // create the use on pickup toggle.
//=============================================================================
    ...
    const width = this._generalContainer.width;
    const w = Math.round( width / 1.5 ) - p * 3;
    const data = [{ name: "self" }, { name: "direct" }, { name: "melee" }];

    this._useOnPickupLabel = new LabelField( 'Force use:' );
    this._useOnPickupLabel.position.set( p, p + lh * 3 );
    this._useOnPickupLabel.opacity = 100;

    this._generalContainer.addElement( this._useOnPickupLabel );

    this._useOnPickupField = new CheckboxField( false );
    this._useOnPickupField.position.set( p + Math.round( width / 3 ), lh * 3 + p / 2 );
    this._useOnPickupField.disabled = true;
    this._generalContainer.addElement( this._useOnPickupField );

    this._useOnPickupField.setOkHandler( this.onPickupChange.bind( this ) );

  }

//=============================================================================
  onPickupChange( value )
  { // when changing he use on pickup value.
//=============================================================================

    if ( this.item() ) this.item().extras.useOnPickup = value || false;
...
//=============================================================================
    ...
    const x = Graphics.width / 4;
    const width = Graphics.width / 4;
    const height = Math.round( Graphics.height * 0.675 );
...
//=============================================================================
window.Scene_ItemExtras = Scene_ItemExtras;
//=============================================================================
...
//-----------------------------------------------------------------------------
  ...
  //============================================================================
    ...
    const identifier =  /(Scene Item Extras) : Version - (\d+.\d+.\d+)/;
    // $._nameError = 'Scene Item Extras was unable to load! Please revert any changes back to normal!';
